...
//        $GLOBALS 中存放都是全局变量
   ...
   public function parse_name($name, $type=0) {
     ...
     * @param $column_name
     */
    public function getFieldByPK($pk,$column_name){
        $sql  = "select {$column_name}  from {$this->table()} where {$this->fields['PK']} = $pk";
     ...
     */
    public function getPageResult($page,$pageSize,$conditions=''){  //name like '%aa%'
        if(empty($conditions)){  //没有条件 加上一个1=1作为条件
            $conditions = '1=1';
        }
        //>>1.得到总条数
