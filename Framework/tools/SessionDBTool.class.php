...
class SessionDBTool {
   ...
   public function  open($savePath,$sessionName){  //$savePath就是在php.ini中配置的session文件保存到路径,    $sessionName就是发送给浏览器cookie的名字
    ...
    public  function  read($sessionId){  //可以根据 $sessionid找到对应的数据文件
        //p('read....'.$sessionId);
        $sql = "select sess_data from yd_session  where sess_id='$sessionId'";
    ...
    public  function  write($sessionId,$data){  //php代码执行完毕之后将$_SESSION中的数据序列化后写到文件或者是数据库中
        ...
        $sql = "insert into yd_session values('$sessionId','$data',unix_timestamp()) on duplicate key update sess_data='$data',last_modified=unix_timestamp()";
        return $this->db->query($sql);
    }
    public  function  destroy($sessionId){  //根据session_id找到文件或者是数据库记录将其删除
        $sql = "delete from yd_session where sess_id = '$sessionId'";
        return $this->db->query($sql);
    }
    public  function  gc($lifetime){  //垃圾回收机制
        ...
        //>>2.再将其删除
        $sql = "delete from yd_session where  last_modified+$lifetime< unix_timestamp()";
        return $this->db->query($sql);
    }
